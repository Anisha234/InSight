...
class FoundationalCVModel(torch.nn.Module):
    ...
    def __init__(self, backbone, mode='eval', weights=None):
        ...
        if backbone in ['dinov2_small', 'dinov2_base', 'dinov2_large', 'dinov2_giant']:
        ...
        elif backbone in ['convnextv2_tiny', 'convnextv2_base','convnextv2_base_patch', 'convnextv2_tiny_patch','convnextv2_large','convnextv2_384']:
            ...
            backbone_path = {
                ...
                'convnextv2_tiny_patch': 'facebook/convnextv2-tiny-22k-224',
                'convnextv2_base': 'facebook/convnextv2-base-22k-224',
                'convnextv2_base_patch': 'facebook/convnextv2-base-22k-224',
            ...
            if backbone == 'convnextv2_base_patch':
                embed_dim = 1024
            else:
                embed_dim = 768
            print("last_conv")
        ...
        # https://pytorch.org/vision/main/models/generated/torchvision.models.swin_t.html#torchvision.models.swin_t
        ...
        elif backbone in ['resnet18_patch', 'resnet34_patch', 'resnet50_patch', 'resnet101_patch', 'resnet152_patch']:
            # Select the ResNet model based on the 'backbone' parameter
            resnet_models = {
                'resnet18_patch': models.resnet18,
                'resnet34_patch': models.resnet34,
                'resnet50_patch': models.resnet50,
                'resnet101': models.resnet101,
                'resnet152': models.resnet152,
            }
            ...
            self.backbone = nn.Sequential(*list(self.backbone.children())[:-2],self.downsample1, self.downsample2, nn.Flatten())
        else:
    ...
    def forward(self, x):
        ...
        if self.backbone_name in ['convnextv2_base_patch','convnextv2_tiny_patch']:
            features = features['last_hidden_state'] # 1024 16 16
            features =self.downsample1(features)
            features =self.downsample2(features)
     #   if self.backbone_name in ['resnet18_patch','resnet50_patch']:
     #       features =self.downsample1(features)
     #       features =self.downsample2(features)
     #       features =self.downsample3(features)
...
class FoundationalCVModelWithClassifier(torch.nn.Module):
    ...
    """
    def __init__(self, backbone, relu, hidden, num_classes, mode='eval', backbone_mode='eval'):
        """
...
class MultiViewFoundationalCVModelWithClassifier(torch.nn.Module):
    """
    A PyTorch module that combines a foundational computer vision model with a classifier for image classification.
    ...
    For more information on specific foundational CV models, refer to their respective documentation.
    """
    def __init__(self, backbone, hidden, num_classes, num_views, mode='eval', backbone_mode='eval'):
        """
        Initialize the FoundationalCVModelWithClassifier module.
        ...
        - mode (str, optional): The mode of the model, 'eval' for evaluation or 'fine-tune' for fine-tuning. Default is 'eval'.
        """
        super(MultiViewFoundationalCVModelWithClassifier, self).__init__()
        ...
        output_dim = self.calculate_backbone_out()
        output_dim = self.num_views*output_dim
        # Initialize layers as an empty list
        layers = []
        
        # Add the linear layer and ReLU activation if 'hidden' is an integer
        if isinstance(hidden, int):
            layers.append(nn.Linear(output_dim, self.num_views*hidden))
            layers.append(nn.ReLU())
           # layers.append(nn.Dropout(p=0.2))
           # layers.append(nn.BatchNorm1d(hidden))
            output_dim = hidden*self.num_views
    
        # Add the linear layer and ReLU activation for each element in 'hidden' if it's a list
        elif isinstance(hidden, list):
            for h in hidden:
                layers.append(nn.Linear(output_dim, self.num_views*h))
                layers.append(nn.ReLU())
            #    layers.append(nn.Dropout(p=0.2))
            #    layers.append(nn.BatchNorm1d(h))
                output_dim = self.num_views*h
    ...
    def forward(self, x_list):
        ...
        # Pass the input through the backbone
        features=[] #list of all embeddings for the 4 images
        for x in x_list:
            features.append(self.backbone(x))
      
        # Now concat features in groups
        # Initialize an empty list to store the concatenated tensors
        
        x_concat = torch.concat((features[0],features[1], features[2], features[3]), dim=1)



        if self.hidden:
            features = self.hidden_layers(x_concat)
        else:
            features = self.norm(x_concat)
