...
def process_labels(df, col='answer', mlb=None, train_columns=None):
    ...
    if mlb is None:
        ...
        print("DF col dtype", df[col].dtype, col)
        if df[col].dtype == int:
            label = df[col]
        else:
            labels = df[col].apply(lambda x: set(x.split(', ')))
...
class BRSETDataset(Dataset):
    ...
    def __init__(self, df, image_cols, images_dir, label_col, mlb, train_columns, shape=(224, 224), transform=None):
        ...
        print('mBRSET: 0',sum(np.argmax(self.labels, axis=1)==0))
        print('mBRSET: 1',sum(np.argmax(self.labels,axis=1)==1))
...
# Custom Dataset class for PyTorch
class BRSETMultiViewDataset(Dataset):
    """
    Args:
    - df (pd.DataFrame): The DataFrame containing the dataset.
    - image_cols (str): Column name containing the path to the images.
    ...
    dataset = BRSETDataset(df, image_cols='image1', label_col='answer', mlb=mlb, train_columns=train_columns)
    """
    def __init__(self, df, image_col_list, images_dir, label_col, mlb, train_columns, shape=(224, 224), transform=None):

        # Images
        self.image_data =[]
        self.num_multiview = len(image_col_list)
        for idx in range(len(image_col_list)):
            self.image_data.append(df[image_col_list[idx]].values)
        self.images_dir = images_dir
        self.shape = shape
        #print(self.num_multiview, self.images_dir, self.shape, self.image_data)
        self.transform = transform or transforms.Compose([
            transforms.Resize(self.shape),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ...
    def __getitem__(self, idx):
        ...
        # Images:
        imgList=[]
        for k in range(self.num_multiview):
            img_path = self.image_data[k][idx]
            img = Image.open( os.path.join(self.images_dir, img_path) ).convert("RGB")
            img = self.transform(img)
...
class MultiViewMetaDataset(Dataset):
    ...
    dataset = BRSETDataset(df, image_cols='image1', label_col='answer', mlb=mlb, train_columns=train_columns)
    """
    def __init__(self, df, image_col_list,images_dir, meta_col_list, label_col, mlb, train_columns, shape=(224, 224), transform=None, meta_only=False,num_views_per_image=1):

        # Images
        self.image_data =[]
        self.df = df
        self.df_feat = torch.LongTensor(df[meta_col_list].to_numpy()) 
        print(self.df_feat.size())
        self.num_multiview = len(image_col_list)
        self.num_views_per_image = num_views_per_image
        print(image_col_list)
        for idx in range(len(image_col_list)):
            self.image_data.append(df[image_col_list[idx]].values)
    ...
    def __getitem__(self, idx):
        ...
        if self.meta_only:
             return {
            'image': None,
            'meta': torch.LongTensor(self.df_feat[idx]),
            'labels': torch.FloatTensor(self.labels[idx])
        }
        else:
            imgList=[]
            for k in range(self.num_multiview):
                img_path = self.image_data[k][idx]
                img = Image.open( os.path.join(self.images_dir, img_path) ).convert("RGB")
                for i in range(self.num_views_per_image):
                    img2 = self.transform(img)
...
class MultiViewMultiLabelMetaDataset(Dataset):
    """
    Taking in metadata and one image - outputs conditions (including some from metadata)
    """
    def __init__(self, df, image_col_list,images_dir, meta_col_list, label_col_list,shape=(224, 224), transform=None, meta_only=False,num_views_per_image=1):
        ...
        #self.train_columns = train_columns
        #self.labels = process_labels(df, col=label_col, mlb=mlb, train_columns=train_columns).values
        self.df_labels = torch.LongTensor(df[label_col_list].to_numpy()) 
        if meta_col_list:
            self.df_feat = torch.LongTensor(df[meta_col_list].to_numpy()) 
            print(self.df_feat.size())
        else:
            self.df_feat = self.df_labels 
    ...
    def __getitem__(self, idx):
        ...
        if self.meta_only:
            ...
            imgList=[]
            for k in range(self.num_multiview):
#                img_path = self.image_data[k][idx].split('.')[0] + '.jpg'
                img_path = self.image_data[k][idx]
                img = Image.open( os.path.join(self.images_dir, img_path) ).convert("RGB")
                for i in range(self.num_views_per_image):
                    img2 = self.transform(img)
            ...
            return {
                'image': imgList,
                'meta': torch.LongTensor(self.df_feat[idx]),
                'labels': torch.LongTensor(self.df_labels[idx])
            }

'''
class MultiViewMetaMultiLabelDataset(Dataset):
    """
    Custom PyTorch Dataset for VQA (Visual Question Answering).
    ...
    dataset = BRSETDataset(df, image_cols='image1', label_col='answer', mlb=mlb, train_columns=train_columns)
    """
    def __init__(self, df, image_col_list,images_dir, meta_col_list, label1_col,label2_col, mlb, train_columns, shape=(224, 224), transform=None, meta_only=False,num_views_per_image=1):
        ...
        self.mlb = mlb
        self.train_columns = train_columns
        self.labels = process_labels(df, col=label1_col, mlb=mlb, train_columns=train_columns).values
        self.labels2 = df[label2_col].values
        #print(self.labels.shape)
        if len(self.labels.shape) == 1:
            self.labels = np.expand_dims(self.labels, axis=1)
        if len(self.labels2.shape) == 1:
            self.labels2 = np.expand_dims(self.labels2, axis=1)
    ...
    def __getitem__(self, idx):
        ...
        if self.meta_only:
            ...
            return {
                ...
                'meta': torch.LongTensor(self.df_feat[idx]),
                'labels': torch.FloatTensor(self.labels[idx]),
                'labels2': torch.FloatTensor(self.labels2[idx])
            }
'''        